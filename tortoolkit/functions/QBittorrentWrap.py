...
async def update_progress(
    ...
    while True:
        ...
        if tor_info.size > (get_val("MAX_TORRENT_SIZE") * 1024 * 1024 * 1024):
            await message.edit(
                "Torrent oversized max size is {}. Wait for @xitij2049 to come online and see this message, he'll change the value of MAX_TORRENT_SIZE.".format(
...
async def register_torrent(entity, message, user_msg=None, magnet=False, file=False):
    ...
    if magnet:
        ...
        if torrent.progress == 1 and torrent.completion_on > 1:
            await message.edit(
                "C'mon Nigga, The provided torrent was already completly downloaded."
            )
            return True
        else:
